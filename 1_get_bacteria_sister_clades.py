...
def process_tree(summary_dir, crassvirales_taxa_df, taxa_groups, outdir, run_name, tree_file):
    ...
    if "Bacteria" in ncbi_summary_df.superkingdom.to_list():
        ...
        if rooted_trees:
            ...
            outfile = f"{outdir}/{run_name}/0_mrcas/{cl_id}.txt"
...
def main():
    ...
    # create output dirs
    os.makedirs(f"{args.outdir}/{args.run_name}/0_mrcas", exist_ok=True)
    ...
    part = partial(process_tree, args.summary_dir, crassvirales_taxa_df, taxa_groups, args.outdir, args.run_name)
