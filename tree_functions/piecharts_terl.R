...
setwd("~/encode_home/projects/Bas_phages/")

# read tree
t <- read.tree("terL_sequences_trimmed_merged_10gaps.treefile.rooted")
t$tip.label <- sapply(strsplit(t$tip.label,"|", fixed=T), `[`, 1)

# read annot
taxa_annot <- read.csv('crass_protein_families_scripts/tree_functions/crassphages_taxonomy_terL_and_new.txt', sep = '\t', header = F)
colnames(taxa_annot) <- c("genome", "family", "subfamily", "genus")


# group families to color them later
dat4 <- taxa_annot %>% select(c("genome", "family"))
...
#ggsave("writing/ggtree/repr_terl.png", device="tiff", dpi=300)
#png("writing/ggtree/repr_terl.png")

pdf("writing/ggtree/repr_TerL.pdf", family = "ArialMT", width = 10, height = 10, pointsize = 18)
