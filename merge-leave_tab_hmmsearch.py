...
def process_CL_hits(CLs_hits, genes_strand, CL_id):
    '''
    Function to process the lines for a given OG.
    '''
    # group the hits per genome. First get the unique genome_ids
    genomes_ids = list(set([line[1].split("|")[0] for line in CLs_hits[CL_id]]))
    CL_genomes = {genome_id:list() for genome_id in genomes_ids}

    for hit in CLs_hits[CL_id]:
        genome_id = hit[1].split("|")[0]
        # remove the "\n" from the line. I didn't this before to keep the length of the lines
        #hit[-1] = hit[-1].replace("\n", "")
        CL_genomes[genome_id].append(hit)


    # process only the 2 hits genomes
    for genome, hits in CL_genomes.items():
        #print(genome, hits)
        #print(hits)
            ...
            if hits[0][2] not in ["No hit", "Other hits"] and hits[1][2] not in ["No hit", "Other hits"]:
                ...
                if int(n2) - int(n1) < 3:
                    ...
                    if len(strands) == 1:
                        ...
                        #print(f"strand incongruence, {cl_id} {genome}")
                        hits[0] += ["leave", "strand"]
                        hits[1] += ["leave", "strand"]
...
def main():
    ...
    lines = [line.strip().split("\t") for line in open(args.hmmsearch_tabular_file).readlines()[1:]]
    ...
    CLs_hits = {CL:list() for CL in list(set([line[0] for line in lines if line[0] not in not_aligned]))}
    ...
    for line in filt_lines:
        CLs_hits[line[0]].append(line)


    for CL_id in list(CLs_hits.keys()):
        print(CL_id)
        process_CL_hits(CLs_hits, genes_strand, CL_id)


    partial_func = partial(process_CL_hits, CLs_hits, genes_strand)
    pool = multiprocessing.Pool(processes=1)
    to_write = pool.map(partial_func, list(CLs_hits.keys()))
