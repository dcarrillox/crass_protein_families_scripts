...
def identify_crassvirales_mrcas(t, cl_id, taxa_groups, start_cont, faces=False, outgroup=None):
    '''
...
def get_rooted_trees(outgroup_seqs, tree_file, ncbi_summary_df, crassvirales_taxa_df):
    '''
    '''
    rooted_trees = dict()
...
def process_tree(summary_dir, crassvirales_taxa_df, taxa_groups, tree_file):
    ...
    if "Bacteria" in ncbi_summary_df.superkingdom.to_list(): # or "NA" in ncbi_summary_df.superkingdom.to_list():
        ...
        rooted_trees = get_rooted_trees(outgroup_seqs, tree_file, ncbi_summary_df, crassvirales_taxa_df)
        ...
        for outgroup_seq, rooted_tree in rooted_trees.items():
            start_cont = 1
            t, mrcas, cont = identify_crassvirales_mrcas(rooted_tree, cl_id, taxa_groups, start_cont, faces=True, outgroup=outgroup_seq)
        ...
        #           "superkingdoms_sister", "bact_phyla_sister", "bact_order_sister"]
        #
        # # check the number of bacteria leaves
        # bacteria_leaves = check_bacteria_leaves(t)
        # # if there is only one Bacteria, root with it
        # if bacteria_leaves:
        #     if len(bacteria_leaves) == 1:
        #         t.set_outgroup(bacteria_leaves[0])
        #         out_seq = bacteria_leaves[0].name
        #         # call crassvirales MRCAs again
        #         cont = 1
        #         t, mrcas, cont = identify_crassvirales_mrcas(t, cl_id, taxa_groups, cont, faces=True)
        #
        #         for mrca_id, mrca in mrcas.items():
        #             polytomy_seqs, n_seqs_mrca, seqs_mrca, sister_support, n_seqs_sister, seqs_sister, n_seqs_bacteria, bacteria_seqs_sister, superkingdoms_sister, bact_phyla_sister, bact_orders_sister = process_MRCA(mrca)
        #
        ...
        #                             bact_orders_sister])
        #
        #     # otherwise, root each MRCA separately and call its sister clade
        #     else:
        #         for mrca_id, mrca in mrcas.items():
        #             # root the tree with the farthest Bacteria sequence to the mrca
        #             t, out_seq = root_farthest_bacteria_hit(t, mrca)
        #             # assess the sister clade
        #             polytomy_seqs, n_seqs_mrca, seqs_mrca, sister_support, n_seqs_sister, seqs_sister, n_seqs_bacteria, bacteria_seqs_sister, superkingdoms_sister, bact_phyla_sister, bact_orders_sister = process_MRCA(mrca)
        #
        #             to_write.append([mrca_id,
        #                             cl_id,
...
def main():
    ...
    for tree_file in tree_files:
        process_tree(args.summary_dir, crassvirales_taxa_df, taxa_groups, tree_file)
