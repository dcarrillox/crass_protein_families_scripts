...
def run_hmm_hmmsearch(hmm_file):
    outdir = "/home/danielc/projects/Bas_phages/5_nr_screening/0_hmmsearch_raw"

    start = time.time()
    CL_id = os.path.basename(hmm_file).split(".hmm")[0]
    os.system(f"hmmsearch --cpu 5 --noali --notextw -o {outdir}/{CL_id}.hmmsearch --domtblout {outdir}/{CL_id}.domtblout {hmm_file} /home/danielc/NOBINFBACKUP/databases/nr_21-11-2021/nr")
    print(CL_id)
    timeit(start)
    print()

print(f"{len(hmms_to_run)} profiles will be analyzed")
