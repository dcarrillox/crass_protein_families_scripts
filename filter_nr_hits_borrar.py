...
def get_lcas(table, faa_dir, outdir, cl_id):
    ...
    if n_included > 0:
        ...
        if os.path.isfile(faa_file):
            ...
            # if necessary, get only the first 3k hits.
            cont = 0
            prots_no_euk_arc = list()
            for protein_id in table.index:
                if cont < 3000:
                    if table.loc[protein_id, "included"]:
                        prots_no_euk_arc.append(protein_id)
                        cont += 1
                else:
                    table.loc[protein_id, "included"] = False


            # get the records for the filtered proteins
            records_no_euk_arc = [record for record in records if record.id in prots_no_euk_arc]
